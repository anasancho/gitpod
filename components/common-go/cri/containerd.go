...
func (s *ContainerdCRI) start() {
	...
	for {
		...
		ccs, err := s.Client.Containers(ctx)
		if err != nil {
			log.WithError(err).Error("cannot list container")
			s.errchan <- xerrors.Errorf("cannot list container: %w", err)
			time.Sleep(reconnectionInterval)
		...
		for _, c := range ccs {
			...
			if err != nil {
				log.WithError(err).Warn("cannot get container's task", err)
				continue
			}
			s.handleNewTask(c.ID(), nil, t.Pid())
		}
...
func (s *ContainerdCRI) handleNewTask(cid string, rootfs []*types.Mount, pid uint32) {
	...
	if !ok {
		...
		log.WithField("containerID", cid).Debug("got no container info for this task - not handling")
		return
	}
	if info.SeenTask {
